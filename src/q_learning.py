...
def hash(obs):
    """
    Encode the local observation window into a compact integer.
    Codes:
...
######### AI Citation ##########
# - I used ChatGPT for debugging my algorithm implementation throughout, starting 
#   with training episodes of 100 to identify issues, etc.
# - I also used ChatGPT to help me minimize runtime, for example I originally was using
#   a Pandas DataFrame structure to keep track of episode rewards (to answer the assignment
#   questions), but then changed that to a Numpy structure to maximize efficiency.
...
########## AI CITATION ##########
# - I used ChatGPT for the below helper method.  This method is just helping to determine
#   if Pacman is standing on a capsule or sharing a tile with a ghost. If so, return that context.
...
if not train_flag:
    ...
    action_hist_by_ctx = {c: np.zeros(env.action_space.n, dtype=np.int64) for c in contexts_order}
    ...
    for episode in tqdm(range(100)):
        ...
        while not done:
            ...
            if gui_flag:
                refresh(obs, reward, done, info, delay=.1)
